...
public class MankiniCreeper extends Creeper {

	public MankiniCreeper(EntityType<? extends MankiniCreeper> type, Level level) {
		super(type, level);
	}
			...
			if (this.level() instanceof ServerLevel serverLevel) {
				if (hitPlayer.getX() == (int) this.getX() || hitPlayer.getY() == (int) this.getY() || hitPlayer.getZ() == this.getZ()) {
					...
					if (full && itemstack != creeperKini) {
						if (MankiniConfig.COMMON.CreeperOverride.get()) {
							...
							if (MankiniConfig.COMMON.EvilCreepers.get()) {
								...
								if (hitPlayer.getInventory().getFreeSlot() == -1) {
									hitPlayer.spawnAtLocation(serverLevel, oldArmour, 0.5F);
								} else {
						...
						hitPlayer.spawnAtLocation(serverLevel, creeperKini, 0.5F);
					}
				}
			}
		}
	...
	@Override
	public boolean checkSpawnRules(LevelAccessor level, EntitySpawnReason spawnReasonIn) {

...
public class MankiniSpider extends Spider {

	public MankiniSpider(EntityType<? extends MankiniSpider> type, Level level) {
		super(type, level);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(4, new MankiniSpider.AttackGoal(this));
		this.goalSelector.addGoal(5, new WaterAvoidingRandomStrollGoal(this, 0.8D));
		this.goalSelector.addGoal(6, new LookAtPlayerGoal(this, Player.class, 8.0F));
		this.goalSelector.addGoal(6, new RandomLookAroundGoal(this));
		this.targetSelector.addGoal(1, new HurtByTargetGoal(this));
		this.targetSelector.addGoal(2, new MankiniSpider.TargetGoal<>(this, Player.class));
		this.targetSelector.addGoal(3, new MankiniSpider.TargetGoal<>(this, IronGolem.class));
	}
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance difficultyIn,
		...
		if (random.nextInt(100) == 0) {
			...
			if (random.nextInt(20) < 5) {
				entitySkeleton = new MankiniSkeleton(ModRegistry.MANKINI_SKELETON.get(), serverLevelAccessor.getLevel());
			}
			...
			MankiniCreeper mankiniCreeper = new MankiniCreeper(ModRegistry.MANKINI_CREEPER.get(), serverLevelAccessor.getLevel());
			mankiniCreeper.snapTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
		...
		if (spawnDataIn == null) {
			...
			if (difficultyIn.getDifficulty() == Difficulty.HARD && random.nextFloat() < 0.1F * difficultyIn.getSpecialMultiplier()) {
				((MankiniSpider.SpiderEffectsGroupData) spawnDataIn).setRandomEffect(random);
			}
		}

		if (spawnDataIn instanceof MankiniSpider.GroupData) {
			Holder<MobEffect> potion = ((MankiniSpider.GroupData) spawnDataIn).effect;
			if (potion != null) {
